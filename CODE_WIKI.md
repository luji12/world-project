...
- 多世界书架与独立世界目录。
- Canon Engine 将用户脚本固化为世界圣经、主线轨道、硬约束和冲突账本。
- 自动推演和玩家介入共用同一事件历史。
...
│   ├── canon_engine.py       # Canon 原文保存、编译与世界包规范化
│   ├── canon_context.py      # 每轮注入给 Agent 的 Canon Packet
│   ├── canon_validator.py    # 玩家行动 / Agent 输出的约束校验与轻修复
│   ├── canon_migration.py    # 旧世界备份、重编译与强制重开
...
- SSE 流式响应。
- 世界创建/切换/删除。
- Canon 状态、原文、世界圣经、冲突、重编译与重开 API。
- 聊天历史读取/清理。
- 章节、正文、导出、存档 API。
...
1. 读取当前世界、预算上下文与 Canon Packet。
2. 规划 NPC 生命周期。
3. 执行世界、系统、主角/NPC、记录员。
4. 将输出归一化，避免模型返回 list/string 时崩溃。
5. 通过 Canon Validator 轻修复跑偏输出或记录冲突。
6. 写入状态、记忆、账本和聊天历史。
7. 通过 SSE 返回前端。
...
| 文件 | 责任 |
| --- | --- |
| `backend/canon_engine.py` | 保存 `source.md`，编译 `world_bible/story_arcs/constraints/source_map/conflicts`，并用 Canon 规范化世界包。 |
| `backend/canon_context.py` | 给所有 Agent 构建最高优先级 Canon Packet。 |
| `backend/canon_validator.py` | 拦截玩家越阶段行动，修复轻微地点漂移，记录冲突。 |
| `backend/canon_migration.py` | 备份旧世界到 `worlds/_archives/`，清空运行态并按 Canon 重开。 |
...
| `src/pages/Canon.jsx` | 世界圣经面板：原文、结构化 Canon、主线阶段、冲突、重编译和重开。 |
