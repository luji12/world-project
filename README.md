...
- **Canon / 世界圣经约束工程**：新世界会保存原始脚本，编译结构化世界圣经、主线阶段、硬约束和冲突账本；推演时所有 Agent 都优先读取 Canon Packet，避免跑回旧设定或模型自由发挥带偏主线。
- **双模式推演**：
...
| Agent | 责任 |
| --- | --- |
| Canon Engine | 保存原始脚本，编译世界圣经/主线轨道/硬约束，并校验推演输出。 |
...
│   ├── canon_engine.py       # Canon 原文、世界圣经、主线轨道与约束文件
│   ├── canon_context.py      # 给所有 Agent 注入 Canon Packet
│   ├── canon_validator.py    # 玩家行动 / Agent 输出的 Canon 校验与轻修复
│   ├── canon_migration.py    # 旧世界备份与按 Canon 强制重开
...
├── canon/                    # source.md、world_bible、story_arcs、constraints、conflicts
├── memory/                   # 角色记忆与向量库
...
1. 导入文档或对话创建世界时，系统会保留完整原始脚本。
2. 创建世界前，Canon Engine 会把脚本编译为 `worlds/<world>/canon/` 下的世界圣经、主线轨道、约束与来源映射。
3. 每轮推演前，World/System/NPC/Chronicler 都会拿到 Canon Packet；玩家行动若越过阶段门槛，会被转化为失败、延迟或可执行替代方向。
4. Agent 输出若出现轻微跑偏（例如起始地点漂移），会自动修复并记录冲突；严重冲突会进入冲突列表，避免污染关键状态。
5. 旧世界可以在“世界圣经”页面一键备份并按 Canon 重开，聊天、记忆、账本会清空，备份保留在 `worlds/_archives/`。
